...
| 17 | [Deploy a Oracle Cloud + Cloudflare Pages](17-deploy-oracle-y-cloudflare.md) | Guía paso a paso de deploy gratis para siempre: VM Ampere ARM en Oracle Always Free (4 OCPU/24GB RAM), Node 24 + MySQL 8 + PM2 + nginx + Let's Encrypt, mysqldump del local + scp + import, double firewall (VCN + iptables), CORS fail-closed para prod, frontend en Cloudflare Pages con `_redirects` para SPA fallback y `NODE_VERSION=24` |
